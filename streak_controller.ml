...
let new_game n = Streak_model.start ~rank:!Info.streak_rank ~theme:!Info.streak_theme n

let player_move from_x from_y to_x to_y =
  if Move.is_valid from_x from_y to_x to_y (Streak_model.current_position ())
  then (
    if Streak_model.player_move from_x from_y to_x to_y <> true
    then Streak_model.set_streak_ended ())
