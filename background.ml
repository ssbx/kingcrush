...
let init ~renderer =
  let filename = Filename.(concat (concat !Info.base_dir "images") "background1.png") in
  let tex = Img.load_texture renderer ~filename in
  ...
  bg_rect
  := Sdl.Rect.make ~x:(logic_x_center - logic_bg_center) ~y:0 ~w:logic_w ~h:logic_h;
  bg_tex := Some tex
;;
...
let draw ~renderer =
  Sdl.render_copy renderer ~texture:(get_tex ()) ~srcrect:None ~dstrect:(Some !bg_rect)
;;
