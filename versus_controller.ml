...
let player_move from_x from_y to_x to_y =
  if Move.is_valid from_x from_y to_x to_y (Versus_model.current_position ()) then (
    if Versus_model.player_move from_x from_y to_x to_y <> true then
      Versus_model.set_streak_ended ())
