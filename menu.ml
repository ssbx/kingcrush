...
let handle_sdl_button_down (evt : Sdl.MouseButtonEvent.t) =
  if (evt.mb_button = 1) then (
    update_hl evt.mb_x evt.mb_y
  ) else (
    false
  )
...
let draw ~renderer =
  ...
  if !hl_enabled then
    Sdl.render_copy renderer ~texture:(get_hl_tex ()) ~srcrect:None ~dstrect:(Some !hl_rect)
