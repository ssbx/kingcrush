...
module Display = struct
  ...
  let gen_board_rect thin_p large_p =
    ...
    Sdl.Rect.make ~w ~h:w ~x:(logical_w - w - large_p) ~y:thin_p
  ;;

  let board_rect : Sdl.Rect.t = gen_board_rect padd_thin padd_large

  let gen_score_rect thin_p large_p =
    let w = logical_w - (2 * large_p) - thin_p - board_rect.w
    and h = logical_h - (2 * thin_p) in
    Sdl.Rect.make ~w ~h ~x:large_p ~y:thin_p
  ;;

  let score_rect : Sdl.Rect.t = gen_score_rect padd_thin padd_large
  let logical_board_width : int = board_rect.w
