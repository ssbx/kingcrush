...
module Squares = struct
  ...
  let draw ~renderer =
    sdl_try (Sdl.render_copy ~dst:Conf.Display.board_rect renderer (get_tex ()))
end
...
module Hints = struct
  ...
  let init ~renderer =
    let img = Filename.(concat (concat !Conf.base_dir "images") "hint.png") in
    rdr := Some renderer;
...
module Position = struct
  ...
  let update_position () =
    cleanup_view_state ();
    view_state.position <- Conf.model.current_position ();
    update_board_texture view_state.position.board

  let coords_to_square x y =
    let bw = Conf.Display.logical_board_width in
    let pw = bw / 8
  ...
  let rec update_pick () =
    ...
    | BUp (to_r, to_f) :: t ->
        if view_state.drag_active = true then (
          ...
          view_state.drag_queue <- t;
          Hints.clear ();
          Conf.ctrl.player_move view_state.drag_from_rank
    ...
    | BUpCancel :: t ->
        ...
        view_state.drag_queue <- t;
        Hints.clear ();
        update_board_texture (Conf.model.current_position ()).board;
        update_pick ()
  ...
  let handle_button1_down x y =
    ...
    | None -> ()
    | Some (rank, file) ->
        if Conf.model.player_turn () then
          let piece = view_state.position.board.(file).(rank) in
          if Conf.ctrl.can_pick_piece rank file then
  ...
  let handle_mouse_wheel event =
    if Option.is_none view_state.anim_piece then
      ...
      | 1 -> Conf.ctrl.move_backward ()
      | -1 -> Conf.ctrl.move_forward ()
      | _ -> ()
  ...
  let handle_game_event = function
    | Streak.Model.NewPuzzle ->
        update_position ()
    | Streak.Model.PuzzleSolved ->
        update_position ();
        Audio.play Audio.PuzzleRushGood
    | Streak.Model.OponentMove (from_pos, to_pos) ->
        anim_move from_pos to_pos
    | Streak.Model.PlayerMove (_, _) ->
        play_audio ();
        update_position ()
    | Streak.Model.Update -> update_position ()
    | Streak.Model.MoveForward (from_pos, to_pos) ->
        anim_move from_pos to_pos
    | Streak.Model.MoveBackward (from_pos, to_pos) ->
        anim_move from_pos to_pos
    | _ -> ()
  ...
  let init ~renderer =
    ...
    Sdl.Rect.set_w view_state.drag_rect Conf.Display.logical_square_width;
    Sdl.Rect.set_h view_state.drag_rect Conf.Display.logical_square_width;
    Sdl.Rect.set_w view_state.anim_rect Conf.Display.logical_square_width;
    Sdl.Rect.set_h view_state.anim_rect Conf.Display.logical_square_width;
    update_board_texture view_state.position.board

  let draw ~renderer =
    let ptext = get_pieces_text () in
    sdl_try (Sdl.render_copy ~dst:Conf.Display.board_rect renderer ptext);
